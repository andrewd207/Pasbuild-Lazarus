...
{
  This unit provides interfaces and base classes to resolve and
  deserialize PasBuild project structures (POM, libraries, and
  applications) from JSON, using RTTI for mapping and environment
  variable resolution.
}
unit Pasbuild.ResolveIntf;
...
uses
  Classes, SysUtils, fpJson, JsonParser, TypInfo, fpJsonRtti, fgl, Utils.console;
...
type
  ...
  public
    ...
    class function HasPasbuildInfo: Boolean;
  ...
  public
    ...
    property Parent: TProjectPOM read FParent write FParent;
  ...
  published
    ...
    property modules: TModuleList read Fmodules write Fmodules;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        case lJsonItem.Get('projectType', 'unknown') of
          ...
          LogWarning(Format('[lazarus] unexpected project type "%s" for "%s"', [lJsonItem.Get('projectType', 'unknown'), lJsonItem.Get('name', '')]));
          lModule := TCommonPasbuildProject.Create;

        end;
        TJSONDeStreamer(Sender).JSONToObject(lJsonItem, lModule);
        LogInfo('[lazarus.package] Added Module ---- ' + lModule.name);
        TModuleList(lList).Add(lModule);
        lModule.Parent := AObject as TProjectPOM;
      end;
      Continue:=True;
    end;
    ...
    begin
      ...
      begin
        ...
        TDependancyList(lList).Add(lDependancy);
...
begin
  try
    ...
    lProc.Executable:='/home/andrew/.local/bin/pasbuild';
    ...
    case lJson.GetPath('project.projectType').AsString of
      ...
      WriteLn(Format('[WARN][lazarus.package] unexpected project type "%s" for "%s"', [lJson.Get('projectType', 'unknown'), lJson.Get('name', '')]));
...
class function TPasbuildResolve.HasPasbuildInfo: Boolean;
begin
  Result := (ProjectDir <> '') and (ProjectFile <> '');
end;

{ TCommonPasbuildProject }

function TCommonPasbuildProject.Getversion: String;
begin
  Result := Fversion;
